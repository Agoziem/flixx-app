// We commence by building out a router
const API_KEY = "b9ba89c625d0c39cca3fae839eed7038";
...
async function displayShowDetails() {
  ...
  addBackgroundImage('tv', show.backdrop_path);
const div = document.createElement('div');
...
// display backdrop for Movies and Shows details page
function addBackgroundImage(type,backgroundPath) {
...
async function search() {
    ...
    if (global.search.term !== '' && global.search.term !== null) {
        ...
        showAlert('please enter a Movie to search','error');
    }
}
...
async function searchAPIData() {
    showSpinner()
    const response = await fetch(`${API_URL}search/${global.search.type}?api_key=${API_KEY}&language=en-US&query=${global.search.term}&page=${global.search.page}`); /* ? is used for Query String */
...
function displayPagination() {
    ...
    // disable prev button when on the First page
    ...
    // Add eventlisteners to the button to make another API request
...
async function fetchAPI(endpoint) {
    showSpinner()
    const response = await fetch(`${API_URL}${endpoint}?api_key=${API_KEY}&language=en-US`); /* ? is used for Query String */
    hideSpinner();
    const data = response.json();
    return data;
}
